...
#' @param col_names Column names (optional, uses data names if NULL)
...
#' @param extensions DataTables extensions (e.g., "Buttons")
#' @param buttons Button configuration if using Buttons extension
#' @param editable Editable configuration (FALSE or list)
#' @param class CSS class for table (default: "cell-border stripe hover")
#' @param row_callback Row callback JavaScript (optional)
#' @param scrollY Vertical scroll height (optional, e.g., "400px")
#' @param language_override Override default language settings
#'
...
#' \dontrun{
#' # Simple table
#' create_standard_datatable(my_data)
#'
#' # Table with selection and custom columns
#' create_standard_datatable(
#'   my_data,
#'   selection = "single",
#'   col_names = c("ID", "Name", "Category"),
#'   col_defs = list(
#'     list(targets = 0, visible = FALSE)
#'   )
#' )
#'
#' # Table with action buttons (escape = FALSE for HTML)
#' create_standard_datatable(
#'   data_with_buttons,
#'   escape = c(TRUE, TRUE, FALSE),
#'   selection = "none"
#' )
#' }
#'
...
create_standard_datatable <- function(
  ...
  options <- list(
    ...
    dom = dom,
    language = if (!is.null(language_override)) language_override else get_datatable_language(),
    ordering = TRUE,
    autoWidth = FALSE,
    paging = TRUE
  )
 ...
 if (!is.null(col_defs)) {
  ...
  if (!is.null(row_callback)) {
    options$rowCallback <- row_callback
  }

  # Add buttons if provided
  if (!is.null(buttons) && "Buttons" %in% extensions) {
    options$buttons <- buttons
  }

  # Build datatable arguments
  dt_args <- list(
    data = data,
    selection = selection,
    rownames = FALSE,
  ...
  # Add editable if provided
  if (!isFALSE(editable)) {
    dt_args$editable <- editable
  }
