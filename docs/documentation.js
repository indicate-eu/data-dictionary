...
var DocumentationPage = (function() {
  ...
  function reviewingEN() {
    return '<h1>Reviewing & Proposing on GitHub</h1>'
      ...
      + '<p>That default mode only sees the version currently on disk, so versions bumped while nobody '
      + 'snapshotted are missed \u2014 typically contributions merged through \u201cPropose on GitHub\u201d, since CI '
      + 'runs <code>build.py</code> but never commits the index back. To catch up, walk the Git history:</p>'
      + '<div class="doc-mock-modal" style="max-width:100%; padding:12px 16px; font-family:monospace; font-size:12px">'
      + 'python3 snapshot.py --backfill --dry-run<br>'
      + 'python3 snapshot.py --backfill'
      + '</div>'
      + '<p>This reads the <code>version</code> field at every commit that touched each concept set and records the '
      + 'commit that <em>introduced</em> each version \u2014 more accurate than the HEAD approximation, and it recovers '
      + 'intermediate versions the default mode cannot see. Deleted concept sets are included, since a project may '
      + 'still pin one. It only ever <em>adds</em>: a pair already indexed keeps its recorded SHA even when the walk '
      + 'finds a different one, because published SHAs are resolved at runtime by the app and repointing one would '
      + 'change a version someone already cited. Those disagreements are reported as divergences '
      + '(<code>--show-divergences</code> lists them) and left untouched. The scan takes a few seconds, so it stays '
      + 'opt-in and is never called by <code>build.py</code>.</p>'
  ...
  function reviewingFR() {
    return '<h1>Relecture & Proposition sur GitHub</h1>'
      ...
      + '<p>Ce mode par d\u00e9faut ne voit que la version pr\u00e9sente sur le disque : les versions incr\u00e9ment\u00e9es sans '
      + 'que personne ne cr\u00e9e de snapshot sont donc manqu\u00e9es \u2014 typiquement les contributions fusionn\u00e9es via '
      + '\u00ab\u00a0Proposer sur GitHub\u00a0\u00bb, puisque la CI lance <code>build.py</code> mais ne commite jamais l\u2019index. '
      + 'Pour rattraper, parcourez l\u2019historique Git\u00a0:</p>'
      + '<div class="doc-mock-modal" style="max-width:100%; padding:12px 16px; font-family:monospace; font-size:12px">'
      + 'python3 snapshot.py --backfill --dry-run<br>'
      + 'python3 snapshot.py --backfill'
      + '</div>'
      + '<p>Cette commande lit le champ <code>version</code> \u00e0 chaque commit ayant touch\u00e9 le jeu de concepts et '
      + 'enregistre le commit qui a <em>introduit</em> chaque version \u2014 plus pr\u00e9cis que l\u2019approximation par HEAD, '
      + 'et cela r\u00e9cup\u00e8re les versions interm\u00e9diaires invisibles au mode par d\u00e9faut. Les jeux supprim\u00e9s sont '
      + 'inclus, car un projet peut encore en \u00e9pingler un. Elle ne fait qu\u2019<em>ajouter</em>\u00a0: une paire d\u00e9j\u00e0 '
      + 'index\u00e9e conserve son SHA m\u00eame si le parcours en trouve un autre, car les SHA publi\u00e9s sont r\u00e9solus \u00e0 '
      + 'l\u2019ex\u00e9cution par l\u2019application et en repointer un modifierait une version d\u00e9j\u00e0 cit\u00e9e. Ces \u00e9carts sont '
      + 'signal\u00e9s comme divergences (<code>--show-divergences</code> les liste) et laiss\u00e9s intacts. Le parcours '
      + 'prend quelques secondes\u00a0: il reste donc optionnel et n\u2019est jamais appel\u00e9 par <code>build.py</code>.</p>'
